const  chatSaveService  = require("./ChatSaveService");
...
module.exports = function (client, clientManager, chatroomManager) {
  ...
  function handleJoin(chatroomName, callback) {
    const createEntry = () => ({ event: `entrou em ${chatroomName}` })

    let message = "";
    let event = " entrou em ";
    chatSaveService(userNamesel, message, event, chatroomName);
  ...
  function handleLeave(chatroomName, callback) {
    const createEntry = () => ({ event: `saiu de ${chatroomName}` })
